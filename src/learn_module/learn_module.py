...
# CACHE_DIR = '../cache/learn_module'
# STOPWORDS_PATH = './learn_module/stopwords.txt'
...
class LearnSentence(object):
    ...
    def _analyze_info(self, vocab, idx):
        print(list(zip(vocab[0], vocab[1])))
    ...
    def _extract_pattern(self, sentence, doc_type):
        ...
        infos = [idx for idx, tag in enumerate(vocab[1]) if tag in LearnSentence.infowords]
        for idx in infos:
            if self._post_filter(vocab[0][idx], vocab[1][idx], doc_type):
                info = self._analyze_info(vocab, idx)
...
class LearnDiscourse(object):
    ...
    def _paragraph_partition(self, paragraphs_body, doc_type):
        ...
        for paragraph in paragraphs_body:
            ...
            if level > 0:
                paragraph = LearnDiscourse.title_number.format(level) + paragraph[span[1]:]
                if len(pa) > 0:
                    if not flag:
                        flag = True
                        openning = pa.copy()
...
class DocLearnModule(object):
    ...
    def learn_docs(self, doc_type, doc_paths):
        ...
        log.INFO('篇章类型:{}'.format(doc_type))
        log.INFO('篇章列表:{}'.format(json.dumps(doc_paths, ensure_ascii=False)))
        ...
        self._save_data(pattern_doc, os.path.join(CACHE_DIR, 'pattern_doc.json'))
        ...
        template_doc = self.ld.learn_structure(
            ...
            doc_type)
        self._save_data(template_doc, os.path.join(CACHE_DIR, 'template_doc.json'))
...
if __name__ == '__main__':
    ...
    # template = doc_learn.learn_docs('GOVERNMENT_REPORT', [
    #     '../../data/陕西省政府工作报告/2022年政府工作报告.docx',
    #     '../../data/陕西省政府工作报告/2021年政府工作报告.docx',
    #     '../../data/陕西省政府工作报告/2020年政府工作报告.docx',
    #     '../../data/陕西省政府工作报告/2019年政府工作报告.docx',
    #     ])
    # template = doc_learn.learn_docs('GOVERNMENT_REPORT', [
    #     '../../data/泰安市政府工作报告/2021年政府工作报告.docx',
    #     '../../data/泰安市政府工作报告/2020年政府工作报告.docx',
    #     '../../data/泰安市政府工作报告/2019年政府工作报告.docx',
    #     '../../data/泰安市政府工作报告/2018年政府工作报告.docx',
    #     ])
    sentence_learn = LearnSentence()
    vocab = sentence_learn.lac.run('“十三五”末，全市地区生产总值达到2766.5亿元，年均增长5.8%，按可比价格计算，比2010年翻一番')
    sentence_learn._analyze_info(vocab, 1)
